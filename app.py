...
def extract_whois_info(whois_data):
    ...
    cidr = (
        cidr_pattern.search(whois_data).group(1)
        if cidr_pattern.search(whois_data)
        else "N/A"
    )
    address = (
        address_pattern.search(whois_data).group(1)
        if address_pattern.search(whois_data)
        else "N/A"
    )
    regdate = (
        regdate_pattern.search(whois_data).group(1)
        if regdate_pattern.search(whois_data)
        else "N/A"
    )
    updated = (
        updated_pattern.search(whois_data).group(1)
        if updated_pattern.search(whois_data)
        else "N/A"
    )
    admin = (
        admin_pattern.search(whois_data).group(1)
        if admin_pattern.search(whois_data)
        else "N/A"
    )
    tech = (
        tech_pattern.search(whois_data).group(1)
        if tech_pattern.search(whois_data)
        else "N/A"
    )
    abuse = (
        abuse_pattern.search(whois_data).group(1)
        if abuse_pattern.search(whois_data)
        else "N/A"
    )
    email = (
        email_pattern.search(whois_data).group(1)
        if email_pattern.search(whois_data)
        else "N/A"
    )
    phone = (
        phone_pattern.search(whois_data).group(1)
        if phone_pattern.search(whois_data)
        else "N/A"
    )
    organization = (
        organization_pattern.search(whois_data).group(1)
        if organization_pattern.search(whois_data)
        else "N/A"
    )
    descr = (
        descr_pattern.search(whois_data).group(1)
        if descr_pattern.search(whois_data)
        else "N/A"
    )
...
def get_screenshot(url):
    driver = webdriver.PhantomJS(
        executable_path="static/img/phantomjs.exe", service_log_path=os.path.devnull
    )
    driver.get(url)
...
def analyze_url(data):
    ...
    behavior_info = interpret_results(
...
def analyze_hash(data):
    ...
    harmless = data["data"]["attributes"]["last_analysis_stats"]["harmless"]
    timeout = data["data"]["attributes"]["last_analysis_stats"]["timeout"]
    confirmed_timeout = data["data"]["attributes"]["last_analysis_stats"][
        "confirmed-timeout"
    ]
    failure = data["data"]["attributes"]["last_analysis_stats"]["failure"]
    type_unsupported = data["data"]["attributes"]["last_analysis_stats"][
        "type-unsupported"
    ]
    details = data["data"]["attributes"]["last_analysis_results"]
    behavior_info = interpret_results(
...
def analyze_ip(data):
    ...
    undetected_downloaded_samples = data["data"]["attributes"].get(
    ...
    analyze_data = {
        ...
        "undetected_downloaded_samples": undetected_downloaded_samples,
...
def scan_url():
    ...
    if response.status_code == 200:
        ...
        return redirect(
            url_for(
                "error",
                message=f"URLのスキャン中にエラーが発生しました: {response.status_code}",
            )
        )
...
def scan_file():
    ...
    if ck != "on":
        ...
        if response.status_code == 200:
            ...
            return redirect(
                url_for("get_results", analysis_id=analysis_id, scan="file")
            )
        else:
            return redirect(
                url_for(
                    "error",
                    message=f"ファイルのスキャン中にエラーが発生しました: {response.status_code}",
                )
            )
    else:
...
def scan_ip_domain():
    ...
    if user_input == "":
        return redirect(
            url_for("error", message="IPアドレスやドメインを入力してください")
        )
        ...
        if validators.url(user_input):
            ...
            if is_domain_name(domain):
                ...
                return render_template(
                    "error.html", error="有効なドメイン形式ではありません。"
                )
        else:
    ...
    if scan == "url":
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                return redirect(
                    url_for("error", message=f"エラー: {response.status_code}")
                )
    # ファイル
    elif scan == "file":
            ...
            if response.status_code == 200:
                ...
                if "data" in result and "attributes" in result["data"]:
                    if "last_analysis_stats" in result["data"]["attributes"]:
                        analysis_stats = result["data"]["attributes"][
                            "last_analysis_stats"
                        ]
                        if (
                            analysis_stats["malicious"] > 0
                            or analysis_stats["undetected"] > 0
                        ):
                ...
                return redirect(
                    url_for(
                        "error",
                        message=f"エラー:ファイルのデータが見つかりませんでした {response.status_code}",
                    )
                )
    # IPアドレス,ドメイン
    elif scan == "ip_domain":
        if analysis_id == "ip":
            url = (
                f'https://www.virustotal.com/api/v3/ip_addresses/{ip_domain_data["ip"]}'
            )
        else:
        ...
        if response.status_code == 200:
            ...
            return redirect(
                url_for(
                    "error",
                    message=f"エラー:情報の取得に失敗しました {response.status_code}",
                )
            )
